...
public class ProyectoIntegradorP2_Equipo10 {
    ...
    try { //Leer archivo
        ...
        //Inicializar objetos en null
        etiqueta.setetiqueta(null);
        codop.setcodop(null);
        operando.setoperando(null);
            ...
            //Leer palabras 
                ...
                for(String Palabra : Palabras) {
                    ...
                    if(Palabra.endsWith(":")) {
                        ...
                        etiqueta.setetiqueta(Palabra); //La palabra identificada se guardara en el objrto etiqueta
                        } //Fin de if
                        else {
                            System.out.println("Error de Etiqueta: La etiqueta '" + etiqueta.getetiqueta() + "' excede la longitud maxima de 8 caracteres."); 
                            etiqueta.setetiqueta("Error"); // Restablecer etiqueta solo si excede la longitud máxima 
                        } //Fin de else 
                    } // Fin de if                                                                      

                    //Las variables ya estan inicializadas en null, por lo tanto siempre entra a la condicion
                    //Validacion CODOP
                    else if(codop.getcodop() == null) { //Si el codigo operando es igual a null                        
                        codop.setcodop(Palabra); //La palabra identificada se guardara en el objeto Codop
                        ...
                        if (!matcher.matches() && codops(codop.getcodop()) != true) { // Verificar si la palabra no cumple con el patrón
                           System.out.println("Error Codop: El Codop '" + codop.getcodop() + "' excede la longitud maxima de caracteres o contiene un simbolo invalidado"); //Si no cumple el patron entonces manda un mensaje de error
                           codop.setcodop("Error"); //Mensaje de error
                        } //Fin de if       
                    } //Fin de else if
                    ...
                    else if(operando.getoperando() == null) { //Si el operando es igual a null
                            operando.setoperando(Palabra); //La palabra identificada se guardara en el objeto Operando
                        } //Fin de else if                       
                    } //Fin de for                                         

                   // Validar espacios en blanco o tabuladores en Etiqueta, CODOP y Operando
                   if (etiqueta.getetiqueta() != null && (etiqueta.getetiqueta().contains(" ") || etiqueta.getetiqueta().contains("\t"))) {
                       System.out.println("Error Etiqueta: La etiqueta contiene espacios en blanco o tabuladores");
                       etiqueta.setetiqueta(null); // Restablecer etiqueta si es inválida
                   } //Fin de if                  

                //Impresion de las variables
                System.out.println("ETIQUETA = " + etiqueta.getetiqueta());
                System.out.println("CODOP = " + codop.getcodop());
                System.out.println("OPERANDO = " + operando.getoperando() + "\n");
            } //Fin de else 
        } //Fin de while       
